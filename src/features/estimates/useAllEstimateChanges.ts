...
async function fetchAll(projectId: string): Promise<FetchResult> {
  ...
  const changes: ChangeRow[] = rows.map((c: any) => ({
    ...c,
...
export function useAllEstimateChanges(projectId: string | undefined) {
  ...
  const queryKey = ["estimateChanges", projectId] as const;
