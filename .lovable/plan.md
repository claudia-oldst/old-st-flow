...
3. Make the Month-to-date window start at 00:00 on the 1st of the month in the company reporting timezone (South Africa), and end at the as-of moment.
4. Make the health epic snapshot use a discount's "applies from" date, matching the portal and the client report.
5. Keep every log at the exact moment it was made, and show each log with the date and time it was entered by that person, so a log entered at 09:00 in Manila is shown as 9 Sep, not shifted into the previous day for a viewer in London.
...
- Timezone: `time_logs.logged_at` is `timestamptz` and pickers build the instant from the logger's local date/time (`combineDateAndTime`), so nothing changes at write time. To keep the entered day stable for readers elsewhere, add `logged_tz_offset` (integer minutes) to `time_logs`, captured on insert from the browser, and render list/group dates using the stored offset rather than the viewer's zone. Client-facing month/day roll-ups stay on the single reporting timezone so totals are consistent.
- Add unit coverage in the existing trend/snapshot tests for a log recorded on the as-of day, a discount applied on the as-of day, and a log made just after midnight in a different timezone.
