...
CREATE OR REPLACE FUNCTION public.list_project_tickets(
  _project_id uuid,
  _filters jsonb DEFAULT '{}'::jsonb,
  _search text DEFAULT NULL,
  _sort_col text DEFAULT 'position',
  _sort_dir text DEFAULT 'asc',
  _page int DEFAULT 1,
  _page_size int DEFAULT 400
)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
...
BEGIN
  ...
  -- Build the filtered set as a CTE we can both count and page
  RETURN (
    WITH base AS (
      SELECT t.*
